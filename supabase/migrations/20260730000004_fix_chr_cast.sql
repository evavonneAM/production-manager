-- chr() takes int; count(*) is bigint — the suffix computation needs a cast.
...
begin
  ...
  select count(*)::int into v_jobs from jobs where project_id = v_project.id;
  select count(*)::int into v_claimed from er_line_items
  ...
  select chr(65 + count(*)::int) into v_next from jobs where project_id = v_project.id;
